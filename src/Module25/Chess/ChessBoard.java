...
public class ChessBoard {
    public ChessPiece[][] board = new ChessPiece[8][8]; // creating a field for a new game
    String nowPlayer;
    ...
    public boolean moveToPosition(int startLine, int startColumn, int endLine, int endColumn) {
        if (checkPos(startLine) && checkPos(startColumn)) {
            ...
            if (board[startLine][startColumn].canMoveToPosition(this, startLine, startColumn, endLine, endColumn)) {
                ...
                if (board[startLine][startColumn].getSymbol().equals("K") ||  // check position for a castling
                        board[startLine][startColumn].getSymbol().equals("R")) {
                    board[startLine][startColumn].check = false;
                }

                board[endLine][endColumn] = board[startLine][startColumn]; // if the figure can be moved, we move it
                board[startLine][startColumn] = null; // set null to the previous cell
    ...
    public void printBoard() {  //print the board in the console
    ...
    public boolean castling7() {
        if (nowPlayer.equals("White")) {
            ...
            if (board[0][7].getSymbol().equals("R") && board[0][4].getSymbol().equals("K") && // check that King and Rook haven't been moved yet
                    board[0][5] == null && board[0][6] == null) {              // and the space between them is free
                if (board[0][7].getColor().equals("White") && board[0][4].getColor().equals("White") &&
                        board[0][7].check && board[0][4].check &&
                        !new King("White").isUnderAttack(this, 0, 6)) { // check that the position for the king is not under attack
                    board[0][4] = null;
            ...
            if (board[7][7].getSymbol().equals("R") && board[7][4].getSymbol().equals("K") && // check that King and Rook haven't been moved yet
                    board[7][5] == null && board[7][6] == null) {              // and the space between them is free
                if (board[7][7].getColor().equals("Black") && board[7][4].getColor().equals("Black") &&
                        board[7][7].check && board[7][4].check &&
                        !new King("Black").isUnderAttack(this, 7, 6)) { // check that the position for the king is not under attack
                    board[7][4] = null;
